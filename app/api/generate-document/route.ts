...
Mark each segment clearly (SEGMENT 1, SEGMENT 2, etc.) so downstream documents — Audio Direction, Edit Plan, Music Cue Sheet — can reference specific segments precisely. A segment marker is followed by nothing but what the narrator says. No parenthetical introducing, framing, or describing the segment — no camera direction, no staging note, no description of where she is standing or what the camera is doing, no music notes, no edit notes — anywhere in the document, before a segment or within it. If a segment is silent (no narration), write the marker and state plainly that this segment carries no spoken words — do not describe the image that fills it. Every line that is present must be spoken language, not written description: read it aloud and it should sound like a person talking, not a document being read. Camera position, staging, and shot description belong in the Cinematography Brief and Shot List, not here — this document must stand on its own as a recording script, usable by a narrator who has never seen the Treatment.
